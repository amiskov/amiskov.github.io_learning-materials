...
Основы работы с событиями: синтаксис, делегирование, создание событий и спобовы вызова и так далее. 
...
## Домашнее задание
Написать кастомный селект, который получает объект с опциями:
...
$('#test').append( select.getElement() );
```
                
Создавать элемент (разметку) нужно только тогда, когда он понадобиться. Не нужно в инициализации сразу создавать
элементы. Создаем селект - генерим верстку только в методе, который за это отвечает.
